...
dx_kernel = toCpu(torch.Tensor([-0.5,0,0.5]).unsqueeze(0).unsqueeze(1).unsqueeze(2))
def dx(v):
	return F.conv2d(v,dx_kernel,padding=(0,1))

dx_left_kernel = toCpu(torch.Tensor([-1,1,0]).unsqueeze(0).unsqueeze(1).unsqueeze(2))
def dx_left(v):
	return F.conv2d(v,dx_left_kernel,padding=(0,1))

dx_right_kernel = toCpu(torch.Tensor([0,-1,1]).unsqueeze(0).unsqueeze(1).unsqueeze(2))
def dx_right(v):
	return F.conv2d(v,dx_right_kernel,padding=(0,1))

# First order derivatives (d/dy)

dy_kernel = toCpu(torch.Tensor([-0.5,0,0.5]).unsqueeze(0).unsqueeze(1).unsqueeze(3))
def dy(v):
	return F.conv2d(v,dy_kernel,padding=(1,0))

dy_top_kernel = toCpu(torch.Tensor([-1,1,0]).unsqueeze(0).unsqueeze(1).unsqueeze(3))
def dy_top(v):
	return F.conv2d(v,dy_top_kernel,padding=(1,0))

dy_bottom_kernel = toCpu(torch.Tensor([0,-1,1]).unsqueeze(0).unsqueeze(1).unsqueeze(3))
def dy_bottom(v):
...
#laplace_kernel = toCpu(torch.Tensor([[0,1,0],[1,-4,1],[0,1,0]]).unsqueeze(0).unsqueeze(1)) # 5 point stencil
#laplace_kernel = toCpu(torch.Tensor([[1,1,1],[1,-8,1],[1,1,1]]).unsqueeze(0).unsqueeze(1)) # 9 point stencil
laplace_kernel = toCpu(0.25*torch.Tensor([[1,2,1],[2,-12,2],[1,2,1]]).unsqueeze(0).unsqueeze(1)) # isotropic 9 point stencil
def laplace(v):
...
map_vx2vy_kernel = 0.25*toCpu(torch.Tensor([[0,1,1],[0,1,1],[0,0,0]]).unsqueeze(0).unsqueeze(1))
def map_vx2vy(v):
	return F.conv2d(v,map_vx2vy_kernel,padding=(1,1))

map_vx2vy_left_kernel = 0.5*toCpu(torch.Tensor([[0,1,0],[0,1,0],[0,0,0]]).unsqueeze(0).unsqueeze(1))
def map_vx2vy_left(v):
	return F.conv2d(v,map_vx2vy_left_kernel,padding=(1,1))

map_vx2vy_right_kernel = 0.5*toCpu(torch.Tensor([[0,0,1],[0,0,1],[0,0,0]]).unsqueeze(0).unsqueeze(1))
def map_vx2vy_right(v):
	return F.conv2d(v,map_vx2vy_right_kernel,padding=(1,1))

map_vy2vx_kernel = 0.25*toCpu(torch.Tensor([[0,0,0],[1,1,0],[1,1,0]]).unsqueeze(0).unsqueeze(1))
def map_vy2vx(v):
	return F.conv2d(v,map_vy2vx_kernel,padding=(1,1))

map_vy2vx_top_kernel = 0.5*toCpu(torch.Tensor([[0,0,0],[1,1,0],[0,0,0]]).unsqueeze(0).unsqueeze(1))
def map_vy2vx_top(v):
	return F.conv2d(v,map_vy2vx_top_kernel,padding=(1,1))

map_vy2vx_bottom_kernel = 0.5*toCpu(torch.Tensor([[0,0,0],[0,0,0],[1,1,0]]).unsqueeze(0).unsqueeze(1))
def map_vy2vx_bottom(v):
	return F.conv2d(v,map_vy2vx_bottom_kernel,padding=(1,1))


mean_left_kernel = 0.5*toCpu(torch.Tensor([1,1,0]).unsqueeze(0).unsqueeze(1).unsqueeze(2))
def mean_left(v):
	return F.conv2d(v,mean_left_kernel,padding=(0,1))

mean_top_kernel = 0.5*toCpu(torch.Tensor([1,1,0]).unsqueeze(0).unsqueeze(1).unsqueeze(3))
def mean_top(v):
	return F.conv2d(v,mean_top_kernel,padding=(1,0))

mean_right_kernel = 0.5*toCpu(torch.Tensor([0,1,1]).unsqueeze(0).unsqueeze(1).unsqueeze(2))
def mean_right(v):
	return F.conv2d(v,mean_right_kernel,padding=(0,1))

mean_bottom_kernel = 0.5*toCpu(torch.Tensor([0,1,1]).unsqueeze(0).unsqueeze(1).unsqueeze(3))
def mean_bottom(v):
